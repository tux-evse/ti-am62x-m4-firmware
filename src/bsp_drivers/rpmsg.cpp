...
RPMsgRcvStatus RPMsg::get_msg(HighToLow& msg, uint32_t timeout_ms) {
    ...
    uint32_t remote_end_point_ ; //FCAM update
    ...
    if (!remote_peer_detected) {
        ...
        remote_end_point = (uint16_t)remote_end_point_; //FCAM update
...
bool RPMsg::send_msg(LowToHigh& msg, uint32_t timeout_ms) {
    if (!remote_peer_detected) {
        ...
        DebugP_log("No rpmsg peer detected \r\n");
        return false;
    }
    ...
    DebugP_log("Send_Msg status=%d buffer[0x%2x, ox%2x] size=%d\r\n", status, encode_buf[0], encode_buf[1], ostream.bytes_written);
