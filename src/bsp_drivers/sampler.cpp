...
// FCAM PP Mutex added
    raw_pp_signal_mutex = xSemaphoreCreateBinaryStatic(&raw_pp_signal_mutex_buf);
...
// FCAM: PP Lastest function added (based on CP)
iec61851::PPSignal Sampler::get_latest_pp_signal() {
    iec61851::PPSignal pp_signal;

    xSemaphoreTake(raw_pp_signal_mutex, portMAX_DELAY);

    if (raw_pp_signal.valid) {
        pp_signal.valid = true;
        // FIXME (aw): the conversion could also be done outside of the mutex
        pp_signal.high = ADC_CHANNEL_2_OFFSET + (ADC_CHANNEL_2_SCALE * raw_pp_signal.high) / (1 << ADC_RESOLUTION);
        //pp_signal.low = 0;
// FCAM Not used        cp_signal.low = ADC_CHANNEL_1_OFFSET + (ADC_CHANNEL_1_SCALE * raw_cp_signal.low) / (1 << ADC_RESOLUTION);
    } else {
        pp_signal.valid = false;
    }

    xSemaphoreGive(raw_pp_signal_mutex);
    //DebugP_log("PP get_laTEST_FLAG_pp_signal valid: %d\r\n", pp_signal.valid); //TMA PP valid = 1
    return pp_signal;
}
...
void Sampler::loop() {
    while (1) {
        ...
        //pp_high_avg.add_value(high_sample.cp);
        //pp_low_avg.add_value(low_sample.cp);

        // NOTE: acquisition cp signal average
        xSemaphoreTake(raw_cp_signal_mutex, portMAX_DELAY);
        ...
        xSemaphoreGive(raw_cp_signal_mutex);
/* TMA CP management*/
        // NOTE: acquisition pp signal average
        xSemaphoreTake(raw_pp_signal_mutex, portMAX_DELAY);
        /*
        if ((pp_high_avg.misfits_count <= PP_SAMPLE_MAX_MISFITS) &&
            (pp_low_avg.misfits_count <= PP_SAMPLE_MAX_MISFITS) && pp_high_avg.filled && pp_low_avg.filled) {*/
        if ((pp_avg.misfits_count <= PP_SAMPLE_MAX_MISFITS) && pp_avg.filled) {
            raw_pp_signal.high = pp_avg.average;
            //raw_pp_signal.low = pp_low_avg.average;
            raw_pp_signal.valid = true;
        } else {
            raw_pp_signal.valid = false;
        }
        //DebugP_log("PP valid Raw value : %d\r\n", raw_pp_signal.valid); // TMA raw_pp_signal.valid = 1
