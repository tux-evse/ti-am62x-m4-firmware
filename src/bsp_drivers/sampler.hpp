...
static constexpr uint16_t PP_SAMPLE_MAX_MISFITS = 1;
...
public:
    ...
    iec61851::PPSignal get_latest_pp_signal();  // FCAM Get Latest PP added
...
private:
    ...
    AveragingRingBuffer pp_avg{PP_SAMPLE_MAX_DEVIATION};  // Only PP High used  PUT IN COMMENT
    //TMA : Check validity return ...in LOW ??
    //AveragingRingBuffer pp_high_avg{PP_SAMPLE_MAX_DEVIATION};  // PP High
    //AveragingRingBuffer pp_low_avg{PP_SAMPLE_MAX_DEVIATION};  // PP Low

    RawCPSignal raw_cp_signal;
    StaticSemaphore_t raw_cp_signal_mutex_buf;
    SemaphoreHandle_t raw_cp_signal_mutex;

    // FCAM duplicate from CP to PP elements
    RawPPSignal raw_pp_signal;
    StaticSemaphore_t raw_pp_signal_mutex_buf;
    SemaphoreHandle_t raw_pp_signal_mutex;
